...
func NewModule(db *gorm.DB, redisClient *goredis.Client, rabbitMQProvider *messaging.RabbitMQRegister, authConfig config.AuthConfig, env config.Env) *Module {
	return &Module{
		db:               db,
...
func (module *Module) Register(group *gin.RouterGroup) {
	...
	{
		auth.POST("/register", handlers.NewRegister(redisRepo, module.rabbitMQProvider, module.userRepository, module.jwtService, module.authConfig, module.env).Handle)
